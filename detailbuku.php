<?php
	include "koneksi.php";
	$db = new database();
	$data_detailbuku = $db->tampil_detailbuku();
		
?>

<html>
<head><title>Detail Buku</title>
	<meta charset="utf-8">
	  ...
	  <div class="card-header bg-info">
		...
		<div class="card-body">
			<?php
				...
				$previous = $halaman - 1;
				$next = $halaman + 1;
				$data = $db->tampil_detailbuku();
				$jumlah_data = $data;
				$total_halaman = ceil($jumlah_data / $batas);
				$data_detailbuku = $db->tampil_detailbuku_paging($halaman_awal,$batas);
				$nomor = $halaman_awal+1;
				if($jumlah_data!=0){
					include "view_detailbuku.php";
				}else{
